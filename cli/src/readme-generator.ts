...
export function generateReadme({
  ...
  if (hasClaude) {
    ...
    sections.push("```");
    sections.push(
      `Reads \`CLAUDE.md\` for instructions. Agents in \`.claude/agents/\`, skills in \`.claude/skills/\`.\n`,
    );
  }
  if (hasOpencode) {
    ...
    sections.push("```");
    sections.push(
      `Reads \`AGENTS.md\` + \`opencode.json\`. Agents in \`.opencode/agents/\`, skills in \`.opencode/skills/\`.\n`,
    );
  }
  if (hasAntigravity) {
    ...
    sections.push(
      `Reads \`GEMINI.md\`. Skills in \`.agent/skills/\`, rules in \`.agent/rules/\`, workflows in \`.agent/workflows/\`.\n`,
    );
  }
  ...
  sections.push(
    `This project includes **144+ specialist AI agents** organized into divisions:\n`,
  );
  sections.push("| Division | Agents |");
  sections.push("|----------|--------|");
  sections.push(
    "| Engineering | Backend, Frontend, DevOps, AI/ML, Security, SRE, Mobile |",
  );
  ...
  sections.push("```bash");
  sections.push("bash scripts/gart.sh sync");
  sections.push("```\n");

  // Docs
  sections.push(`## Documentation\n`);
  sections.push(
    "- `docs/guides/` — Reference guides (debugging, MCP, agent workflows)",
  );
  ...
  if (hasOpencode) {
    sections.push(
      "- `OPENCODE_EXPERIMENTAL_LSP_TOOL=true` — Enable LSP tools for agents",
    );
  }
  sections.push("");
